        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 max-w-5xl mx-auto">
          {seoLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="group rounded-2xl border border-line bg-surface-elevated p-5 text-center text-sm font-semibold text-paper hover:border-gold-500/50 hover:text-gold-300 transition-all duration-300 ease-premium hover:-translate-y-0.5"
            >
              {link.label}
            </Link>
          ))}
